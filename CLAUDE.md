...
**Phase 0 진행 중.** `PLAN.md`(계획서 v0.3)와 `docs/plans/p0-foundation.md`(작업계획서)가 기준 문서다. 구현 전에 둘 다 확인하고, 작업은 `/ted-run` 파이프라인으로 진행한다. 진행 현황은 `HANDOFF.md` 참조.

## 자주 쓰는 명령어

```bash
npm run ci              # typecheck + lint + 테스트·커버리지 (커밋 전 필수)
npm test                # vitest 단위 테스트
npm run test:coverage   # 커버리지 게이트 (lines/branches/functions 80%)
...
- **스타일**: **StyleSheet + `@ted-speak/shared` 디자인 토큰** — NativeWind 아님 (ADR-0001, NativeWind 5 stable 시 재평가). 인라인 hex 금지, 토큰만 사용.
- **Audio**: **expo-audio** (expo-av는 SDK 56에서 deprecated — ADR-0002) + expo-speech, OpenAI Realtime API는 Phase 2 AI 튜터 전용
- **AI**: OpenAI Whisper (STT) + GPT-4o (LLM) + OpenAI TTS — turn-based, TTS는 스트리밍 재생·Drill 채점은 로컬 `scoreDrill()` (ADR-0003)
...
- API 키를 앱 번들에 내장하지 않는다 — `packages/ai` 클라이언트는 호출자가 키 주입 (출시 전 Edge Function 프록시 전환 전제)
- 콘텐츠·피드백 타입은 zod 스키마(`packages/shared/src/{content,feedback}-schema.ts`)가 단일 출처 — 인터페이스 중복 정의 금지, `z.infer`로 파생
- 시드 콘텐츠 추가 시 `npm run validate:content`로 검증
