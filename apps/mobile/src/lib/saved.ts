/**
 * saved.ts — SavedRepo 팩토리 (앱 연결 지점). tutor.ts와 동일 패턴.
...
// mock 모드 영속 저장소: 네이티브는 AsyncStorage, 웹은 메모리 폴백 (tutor.ts와 동일 패턴)
...
let cached: { userId: string; repo: SavedRepo } | null = null;

/**
 * 현재 로그인 사용자의 SavedRepo를 반환한다.
 * supabase 모드인데 미로그인이면 null — 호출 화면은 로그인으로 유도해야 한다.
 */
export function getSavedRepo(): SavedRepo | null {
  ...
  const repo =
    supabase && !user.isMock
      ? createSupabaseSavedRepo(
          supabase as unknown as Parameters<typeof createSupabaseSavedRepo>[0],
          user.id,
        )
      : // mock 저장소는 user.id로 네임스페이스 — 공유 단말에서 사용자 간 데이터 격리(PII)
        createMockSavedRepo(mockStorage, { namespace: user.id });
  cached = { userId: user.id, repo };
  return repo;
}

// 로그아웃 시 캐시 정리 (tutor.ts와 동일 — auth ↔ saved require cycle 방지)
